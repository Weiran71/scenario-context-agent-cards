...
\end{tikzpicture}
\caption{The Context-to-Card architecture. The need record is the explicit interface between context interpretation and service execution.}
...
\begin{tikzpicture}[
  scale=0.68,transform shape,
  node distance=4mm and 7mm,
  box/.style={draw=navy!70,rounded corners=2pt,fill=pale,align=center,minimum height=8mm,text width=30mm,font=\scriptsize},
  gate/.style={draw=navy!70,rounded corners=2pt,fill=orange,align=center,minimum height=8mm,text width=30mm,font=\scriptsize},
  outbox/.style={draw=navy!70,rounded corners=2pt,fill=green,align=center,minimum height=8mm,text width=30mm,font=\scriptsize},
  arr/.style={-{Latex[length=1.6mm]},thick,draw=navy!80}
]
...
Generation is invoked only after \serve{}. The card output is separated into user-facing fields (main copy, sub-copy, Action or POI name) and internal fields (need record, evidence, theme, decision reasons). This separation allows concise presentation and detailed audit simultaneously.
...
Coverage is defined over factors that can change a decision or output: time of day, weather, local versus visiting status, preference combinations, Event themes, POI themes, candidate distance, candidate count, ordinary versus special dates, and positive versus negative polarity. The aim is not to enumerate every Cartesian product. Instead, each factor is varied enough to expose a different reasoning boundary, while avoiding a synthetic distribution in which every Case contains a special date or a single repeated template.
...
The reference validator checks JSON parseability, required top-level keys, card-type and decision enumerations, expected/actual decision agreement, null card content for silence, duplicate IDs, and a small set of public-boundary tokens. It does not prove semantic relevance, factual provenance, Summary quality, or naturalness. Those limitations are part of the release contract.
...
The dataset is synthetic and cannot establish performance on real users, places, or distributions. Thresholds and weights are illustrative. Automatic checks cover structure and selected hard boundaries but not semantic truth. A real deployment would require authorized data, privacy minimization, security review, calibration against real distributions, human or model evaluation of language, and online safety monitoring.
